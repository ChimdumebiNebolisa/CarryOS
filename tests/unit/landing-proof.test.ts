...
describe('landing proof', () => {
  it('derives reconciliation from the deterministic closed-bag scan', () => {
    ...
    expect(proof.activity.name).toBe('Calculus II')
    expect(proof.rows.filter((row) => row.relationship === 'matched')).toHaveLength(3)
    expect(proof.rows.find((row) => row.itemId === 'calculator')?.relationship).toBe('missing')
    expect(proof.alert?.evidence.itemName).toBe('Calculator')
  })
})
