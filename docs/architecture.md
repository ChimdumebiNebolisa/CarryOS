...
`/` is a static editorial poster (split paper type column + stored commute photograph). `/how-it-works` renders an interactive React Flow system graph: five subsystem groups (context, requirements, observation, state, decision), trust-boundary styling (dashed terracotta = model-assisted, sand = user gate, hairline = deterministic, dotted green = simulated input), node selection with a side explanation panel, and a simulation disclosure. `/demo` calls `src/application/demo-scenario.ts` and `src/domain/*` directly.
